...
void PagosArchivo::mostrarTodoElArchivo(){
Pago pago;
FILE *punteroArch;
PagosArchivo pagoArchivo;
int cantidad = pagoArchivo.getCantidad();

punteroArch = fopen("pagos.dat","rb");
if (punteroArch == NULL) exit(1);

for ( int j = 0; j < cantidad;j++) {
fread(&pago,sizeof(Pago),1,punteroArch);
pago.mostrarTodo();
cout <<pago.esPagoVencido();
cout <<"----------------------------------------------" << endl;
}
